...
class DragTimeCalculation(
    ...
    //private late init var postProc: PostProcessing
    private val zeroThreshold = 0.3f // Adjust based on your data
    ...
    private fun postProcessing() {
        ...
        if (session != null) {
            postProc.postProcessing(session)
        }*/
    }

    suspend fun timeFromZeroToHundred(): Double {
        // Fetch session data in chronological order.
        var sessionItems = database.rawGPSDataDao().getGPSDataBySession(session)


        // If no data exists, run postProcessing and re-fetch.
        if (sessionItems.isEmpty()) {
            postProcessing()
            sessionItems = database.rawGPSDataDao().getGPSDataBySession(session)
        }
